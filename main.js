...
const FORM_ENDPOINT = 'https://script.google.com/macros/s/AKfycbyuEGCiWVvBx3BCQf9M4Gaw5sU9tRUfVHkM8GeKTWQFp_U2Sv92cKpaJA3OXi7zWc_zgg/exec';
...
function initForm(){const form=document.getElementById('inquiryForm'),status=document.getElementById('formStatus');if(!form)return;const lang=getLang(),t=FORM_TEXT[lang]||FORM_TEXT.en;const endpointReady=FORM_ENDPOINT&&FORM_ENDPOINT.includes('/macros/s/')&&FORM_ENDPOINT.endsWith('/exec');if(endpointReady){form.action=FORM_ENDPOINT;form.dataset.endpoint='google-apps-script';}function show(type,msg){if(!status)return;status.className='form-status '+type;status.textContent=msg;}function mailto(data){const subject=encodeURIComponent('Raw Materials Trade B2B Inquiry — '+(data.get('product')||data.get('company')||''));let body='';for(const [k,v] of data.entries()){if(v && k!=='consent_gdpr' && k!=='consent_disclaimer' && k!=='website' && k!=='_honey' && !k.startsWith('_')) body+=`${k}: ${v}
`;}window.location.href=`mailto:office@rawmaterialstrade.com?subject=${subject}&body=${encodeURIComponent(body)}`;}form.addEventListener('submit',async e=>{e.preventDefault();if(!form.checkValidity()){form.reportValidity();return}const gdpr=document.getElementById('consentGdpr'),disc=document.getElementById('consentDisclaimer');if(gdpr&&!gdpr.checked){show('error',t.gdpr);return}if(disc&&!disc.checked){show('error',t.disc);return}const data=new FormData(form);data.append('submitted_at',new Date().toISOString());data.append('page_url',window.location.href);data.append('page_title',document.title||'');data.append('language',lang);data.append('form_source','rawmaterialstrade.com');show('info',t.sending);if(endpointReady){try{await fetch(FORM_ENDPOINT,{method:'POST',body:data,mode:'no-cors',keepalive:true});show('success',form.dataset.successMsg||t.sent);form.reset();return}catch(err){mailto(data);show('info',t.fail);return}}mailto(data);show('info',form.dataset.mailtoMsg||t.mailto);});}
